...
description: "记一下我用 Hugo + PaperMod 把这个博客从零搭起来的完整过程——部署到 GitHub Pages、绑自己的域名、接 Giscus 评论。"
ShowToc: true
TocOpen: false
---
...
引用图片时只写文件名，不要带任何路径前缀：

```markdown
✅ ![](nginx-502.png)
❌ ![](/posts/xxx/nginx-502.png)   # 绝对路径，目录结构或域名变动后会全部失效
```

文件名大小写要严格一致。比如 `Screenshot.png` 引用成 `screenshot.png`，在 macOS 默认文件系统（大小写不敏感）下能正常显示，但到 GitHub Actions 的 Linux 环境会 404，这类问题本地无法复现。

要控制图片尺寸，直接写 HTML（前面 `unsafe = true` 已经允许了）：
